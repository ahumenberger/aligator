...
If[$Notebooks,CellPrint[Cell[#,
"Print",FontColor->RGBColor[0,0,0],CellFrame->0.5,Background->RGBColor[0.796887,0.789075,0.871107]]]&,Print]["Aligator.m \n"<>"Automated Loop Invariant Generation by Algebraic Techniques Over the Rationals.\n"<>
...
Aligator[c_,IniVal->{seq_}]:=Module[{invariants={},givenIniRecs,givenIniRules},
...
Aligator[c_]:=Module[{sw,ifCheck,loops,invariants={}},
sw=InputCheck[c];
If[sw==0,Abort[]];
...
AssgnCheck[CompoundExpression[],assgnlist_]:=assgnlist
...
AssgnCheck[CompoundExpression[lhs_:=rhs_],assgnlist_]/;lhs==rhs:=0


AssgnCheck[lhs_:=rhs_,assgnlist_]/;lhs==rhs:=0
...
AssgnCheck[CompoundExpression[lhs_:=rhs_],assgnlist_]:=AssgnCheck[CompoundExpression[],assgnlist]


AssgnCheck[lhs_:=rhs_,assgnlist_]:=AssgnCheck[CompoundExpression[],assgnlist]


AssgnCheck[seq___,assgnlist_]:=0


LoopCondCheck[cond_,condcheck_]/;(ToString[Hold[cond]]==ToString[Hold[Null]]):=0


LoopCondCheck[CompoundExpression[b1_,b2__],condcheck_]:=0


(*put check for :=-free conditions *)


LoopCondCheck[cond_,condcheck_]:=condcheck
...
LoopBodyCheck[body_,bodycheck_]/;(AssgnCheck[body,1]==1):=bodycheck


LoopBodyCheck[IF[bif_,s1_],bodycheck_]/; (LoopCondCheck[bif,1]==1):=LoopBodyCheck[s1,bodycheck]


LoopBodyCheck[IF[bif_,s1_,s2_],bodycheck_]/; (LoopCondCheck[bif,1]==1) := Module[{thencheck},thencheck:=LoopBodyCheck[s1,bodycheck];LoopBodyCheck[s2,thencheck]]


LoopBodyCheck[CompoundExpression[s0__,IF[bif_,s1_]],bodycheck_]/;(AssgnCheck[CompoundExpression[s0],1]==1) :=LoopBodyCheck[IF[bif,s1],bodycheck]


LoopBodyCheck[CompoundExpression[s0__,IF[bif_,s1_,s2_]],bodycheck_]/;(AssgnCheck[CompoundExpression[s0],1]==1) :=LoopBodyCheck[IF[bif,s1,s2],bodycheck]


LoopBodyCheck[CompoundExpression[IF[bif_,s1_],s3__],bodycheck_]/;(AssgnCheck[CompoundExpression[s3],1]==1) :=LoopBodyCheck[IF[bif,s1],bodycheck]


LoopBodyCheck[CompoundExpression[IF[bif_,s1_,s2_],s3__],bodycheck_]/;(AssgnCheck[CompoundExpression[s3],1]==1) :=LoopBodyCheck[IF[bif,s1,s2],bodycheck]


LoopBodyCheck[CompoundExpression[s0__,IF[bif_,s1_],s3__],bodycheck_]/;((AssgnCheck[CompoundExpression[s0],1]==1) && (AssgnCheck[CompoundExpression[s3],1]==1) ):=LoopBodyCheck[IF[bif,s1],bodycheck]


LoopBodyCheck[CompoundExpression[s0__,IF[bif_,s1_,s2_],s3__],bodycheck_]/;((AssgnCheck[CompoundExpression[s0],1]==1) && (AssgnCheck[CompoundExpression[s3],1]==1) ) :=LoopBodyCheck[IF[bif,s1,s2],bodycheck]


LoopBodyCheck[seq__,condcheck_]:=0


InputCheck[WHILE[b_,c___]]/;(LoopCondCheck[b,1]==0):=
Module[{},Message[aligator::InputError1,"Input"];0]


InputCheck[WHILE[b_,c_]]/;(LoopBodyCheck[c,1]==0) :=
Module[{},Message[aligator::InputError2,"Input"];0]
...
InputCheck[WHILE[b_,c1_,c2__]]:=Module[{},Message[aligator::InputError3,"Input"];0]


InputCheck[c___]:=Module[{},Message[aligator::InputError4, "Input"];0]
...
Body[seq1___,Body[]]:=Module[{},Body[seq1]]


Body[Body[],seq2___]:=Module[{},Body[seq2]]


Body[Body[CompoundExpression[seq1___,seq2___]],Body[CompoundExpression[seq3___,seq4___]]]:=Module[{},Body[CompoundExpression[seq1,seq2,seq3,seq4]]]


Body[Body[seq1___],Body[CompoundExpression[seq2___,seq3___]]]:=Module[{},Body[CompoundExpression[seq1,seq2,seq3]]]


Body[Body[CompoundExpression[seq1___,seq2___]],Body[seq3___]]:=Module[{},Body[CompoundExpression[seq1,seq2,seq3]]]


Body[Body[seq1___],Body[seq2___]]:=Module[{},Body[CompoundExpression[seq1,seq2]]]


Body[Body[seq___]]:=Module[{},Body[seq]]


Body[CompoundExpression[s_]]:=Module[{},Body[s]]


CheckIfSeq[loop_]:=Not[FreeQ[Body[loop],IF]]
...
LoopTransform[body_,beforeSt_,afterSt_]/;(FreeQ[Body[body],IF]):=Body[ Evaluate[Body[beforeSt,Evaluate[Body[body]]]], afterSt]


LoopTransform[IF[bif_,s1_],beforeSt_,afterSt_]:={LoopTransform[s1,beforeSt,afterSt],LoopTransform[Body[],beforeSt,afterSt]}
...
LoopTransform[CompoundExpression[s0__,IF[bif_,s1_]],beforeSt_,afterSt_] :=
...
LoopTransform[CompoundExpression[s0__,IF[bif_,s1_,s2_]],beforeSt_,afterSt_] :=
...
LoopTransform[CompoundExpression[IF[bif_,s1_],s3__],beforeSt_,afterSt_]:=
...
LoopTransform[CompoundExpression[IF[bif_,s1_,s2_],s3__],beforeSt_,afterSt_]:=
...
LoopTransform[CompoundExpression[s0__,IF[bif_,s1_],s3__],beforeSt_,afterSt_]:=
...
LoopTransform[CompoundExpression[s0__,IF[bif_,s1_,s2_],s3__],beforeSt_,afterSt_]:=
...
FromLoopToRecs[Body[seq__]]:=Module[{},RecSystem[seq]]
...
RecSystem[List[assgn__]]:=RecSystem[assgn]


RecSystem[assgn__]:=Module[{assgAll,recAssg,n=Unique[],recVars,recEqSystem, recVarList,finalRecEqSystem,finalRecVarList,flattenRecEqSystem,flattenRecVarList,allVars,recChangedVarList,notRecVar,notRecChangedVars,removeVar,normVal,normalizedRecEqSystem,normalizedRecVarList,normedVars,shiftVarsNormed,i},
...
If[recChangedVars=={},Print["No recursively changed variables! Not P-solvable Loop!"];Abort[]];
...
If[shiftVarsNormed<0,normedVars=NormedVarList[normedVars,n,{}]]
];
...
NormedVarList[{},n_,VarList_]:=VarList


NormedVarList[{vars1_,seq___},n_,VarList_]:=Module[{i,minVal},
minVal=Min[Cases[vars1,_[n+i_]->i,Infinity]];
If[minVal<0,NormedVarList[{seq},n,VarList],
...
RecEqs[CompoundExpression[],recSystem_]:=recSystem


RecEqs[CompoundExpression[x_:=rhs_],recSystem_]:=
RecEqs[CompoundExpression[],Join[recSystem,{x==rhs}]]


RecEqs[x_:=rhs_,recSystem_]:=
RecEqs[CompoundExpression[],Join[recSystem,{x==rhs}]]


RecEqs[CompoundExpression[x_:=rhs_,seq__],recSystem_]:=
...
SimplifySeq[{},x_[y_]==expr_,n_,eqs_]:=eqs
...
If[shift<0, system/.(n->n-shift), system ]
]
...
ProperRecVars[{},n_,Vars_]:=Vars


ProperRecVars[{x_[y_]==rhs_,seq___},n_,Vars_]/;RecurrenceOrder[x[y]==rhs,n,x]<=0:=ProperRecVars[{seq},n,Vars]
...
RecurrenceOrder[rec_,n_,f_]:=Max[Cases[rec,f[n+j_.]->j,Infinity]]-Min[Cases[rec,f[n+j_.]->j,Infinity]]
...
RecOrderCheck[rhs_,n_,x_,recOrder_]/;Cases[rhs,x[n+recOrder+j_.]->j,Infinity]!= {} :=0


RecOrderCheck[rhs_,n_,x_,recOrder_]:=1
...
Module[
{depVarsX,indepList,depList,depTerm},
depVarsX=Complement[Union[Cases[rhs,f_[n+j_.]->f,Infinity]],{x}];
If[depVarsX=={},
	(*independent*)
	indepList=Append[indep,x[y]==rhs];depList=dep,
	indepList=indep;depTerm={{x[y]==rhs,depVarsX}};
...
RecDependencyCheck[{},n_,indep_,dep_]:={indep,dep}
...
LinRecCSolve[x_[y_]==rhs_,n_]/;FreeQ[rhs,x]:=Module[{},
Print["Illegal recurrence encountered: " <>ToString[x[y]==rhs]];{False,{},{},{}}]
...
newRec={}; newIniRules={}; sol={}; solExponent={};solSet={};
...
If[Not[cfinite], 
	...
	If[shift==0 , (* inHomPart is 0 -> original rec is CFinite *)  newRec=x[y]==rhs,
...
CFiniteCheck[x_[_],n_,x_]:={True,0}


CFiniteCheck[c_*x_[_],n_,x_]:= If[FreeQ[c,n],{True,0},{False,0}]


CFiniteCheck[x_[_]*c_,n_,x_]:= If[FreeQ[c,n],{True,0},{False,0}]
...
Do[rhsPos=xTermPositions[[i]];
    ...
    rhsInHom=rhsInHom-xTerm;
     ...
     If[sw==0,cFinite=False; Continue[]],
...
If[sw==1,
	 ...
	 If[inHomCFinite ==1, 
		(* inHom part is C-finite *)  cFinite=True;inHomPart=Expand[Simplify[rhsInHom]],
		(* inHom part is not C-finite *) cFinite=False; inHomPart=rhs]					
];
{cFinite,inHomPart}
]
...
CheckTerm[x_[_],n_,x_]:=1


CheckTerm[c_*x_[_],n_,x_]/;FreeQ[c,n]:=1


CheckTerm[x_[_]*c_,n_,x_]/;FreeQ[c,n]:=1


CheckTerm[expr_,n_,x_]:=0
...
InhomCCheck[expr_,n_,inHom_]/;FreeQ[expr,n]:=inHom


InhomCCheck[n_,n_,inHom_]:=inHom


InhomCCheck[n_^k_Integer,n_,inHom_]/;k>0:=inHom


InhomCCheck[Sum[expr_,{k_,_Integer,_}],n_,inHom_]/;FreeQ[expr,n]:=inHom
...
InhomCCheck[a_^b_,n_,inHom_]/;(FreeQ[a,n]&& Exponent[Expand[b],n]==1):=inHom


InhomCCheck[a_^k_Integer,n_,inHom_]:=
(* if FreeQ[a,n], then FreeQ[a^k,n], since k is integer. Check happens only if !FreeQ[a,n]. *)
If[!FreeQ[a,n]&&k<0,Print["Unable to verify c-finitenes of "<>ToString[a^k]];0,
InhomCCheck[a,n,inHom]];


InhomCCheck[a_,n_,inHom_]:=0
...
ShiftOrder[expr_,n_]/;FreeQ[expr,n]:=If[Expand[expr]===0,0,1]


ShiftOrder[n_,n_]:=2


ShiftOrder[n_^k_Integer,n_]/;k>0:=1+k


ShiftOrder[expr_,n_]/;FreeQ[Cases[expr,_^e_->e,Infinity],n]:=1+Exponent[expr,n] 
(*expr is poly in n, has no exponentials in n*)


ShiftOrder[Sum[expr_,{k_,_Integer,_}],n_]/;FreeQ[expr,n]:=1+ShiftOrder[expr/.k->n,n]


ShiftOrder[a_*b__,n_]:=Max[1,ShiftOrder[a,n]]*ShiftOrder[Times[b],n]


ShiftOrder[a_+b__,n_]:=ShiftOrder[a,n]+ShiftOrder[Plus[b],n]


ShiftOrder[a_^b_,n_]/;FreeQ[a,n]:=1


ShiftOrder[a_^k_Integer,n_]:=Binomial[ShiftOrder[a,n]-1+k,k]
...
Module[
{f,factors,polyCoeff,eq,newRhs,simpleRhs,coeffFactorJ,j},
f=L[[1]];factors=L[[2]];polyCoeff=L[[3]];
newRhs=0;
Do[
...
HypergeometricQ[term_,n_]:=Catch[fastZeil`Private`ReleaseHold[Q[term,n]]]=!={};
...
n=recVar;newRecSystem=recSystem;cfSystem={};expList={};iniVarList={};
cfRules={};recsLeft=True;solvable=True;finVars={};
While[ solvable && recsLeft,
	...
	If[indepNr==0 && depNr!=0,
                     ...
                     solvable=False; cfSystem={};expList={};Continue[]]; 
         If[indepNr==0 && depNr==0,
                    recsLeft=False;Continue[]];
        (* there are still independent recs *)
	Do[
		recEq=indepList[[i]];
		rhsEq=recEq[[2]]; x=Union[Cases[recEq,f_[n+j_.]->f,Infinity]][[1]];
		(*RecType Check *)
		...
		If[Not[solvable], cfSystem={};expList={};Continue[],
...
newRecSystem={};expVars={};expDepList={};expBases={};
If[Not[solvable],Print["Not P-solvable loop!"];Abort[]];
...
If[Not[polyCoeff], Print["Not P-solvable loop! Not polynomial closed forms!"]; Abort[]],
{i,1,Length[cfSystem]}
];
...
If[expBases== {},(*Psolvable*)
	...
	(*Psolvable*)
		...
		expBases=cfSystem[[i,3]];
		coeffTerm=0;
		If[expBases== {}, newRecEq=recEq,
			 (* otherwise, it has exponentials *)
			Do[
			    coeffList=solSet[[3,j]];
			    If[solSet[[2,j]]==1, coeffTerm=coeffTerm+Simplify[Sum[coeffList[[u]]*n^(u-1),{u,1,Length[coeffList]}]],
			   	k=k+1; coeffTerm=coeffTerm+Simplify[Sum[coeffList[[u]]*n^(u-1),{u,1,Length[coeffList]}]]*y[k]],
			{j,1,Length[solSet[[2]]]}
		      ];
...
Which[
	 recOrder<1,Print["Not supported rec: ",recEq]; solvable=False,
	 recOrder >1, (* Print["start LinRecCSolve."];*){solvable,cfRec,solSet,exps}=LinRecCSolve[recEqSolved,n];
Print[solvable];
					If[solvable, {cfRec,solSet}={ShiftBackClosedForm[cfRec,n,needShift],ShiftBackSolSet[solSet,n,needShift]}],
	recOrder==1, 
...
InvLoopCond[loopList_]:=Module[
{closedFormsList,innerLoopPerm,allLoopVars,innerLoop,innerLoopRules,innerFinVars,t=Unique[],perm,invIdeal={},i,permIdeal={},sumIntersectionIdeal={},appendableCF,appendablePerms},
...
If[(Complement[invIdeal,invariants]=={}) && (Complement[invariants,invIdeal]=={}),Print["Method is complete!"],
...
UniformInnerLoopVars[{},outerVars_,newClosedFormList_]:=newClosedFormList
...
CfIfLoopSeq[{innerLoop_,innerLoopList___},cfSystemList_,outerLoopVars_]:=Module[
{recSystem,VarList,CFSystem,recVar,expVars,finVars,iniVars,AlgDep,newCFSystem,innerCfSytem},
...
CfIfLoopSeq[{},cfSystemList_,outerLoopVars_]:={cfSystemList,outerLoopVars}
...
CFMerge[{},mergedSys_]:=mergedSys


CFMerge[{seq___,cf_},{{},{},{},{},{},{}}]:=CFMerge[{seq},cf]


CFMerge[{seq___,cf_},mergedSys_]:=Module[
{cfSystem1,recVar1,expVar1,finVar1,iniVar1,algDep1,cfSystem2,recVar2,expVar2,finVar2,iniVar2,algDep2,
...
InvIdealLoopSeq[permSeq_]:=Module[{elimVars,polySys,mergedCF,recVars,expVars,finVars,iniVars, algDeps,invariants},
...
CompletenessCheck[permList_,closedFormsOfPerm_,filteredPolys_,appendableClosedForms_,firstItIdeal_,allLoopVars_]:=Module[
{secondIterationList={},i,perm,loopsToAppend,newLoopSeq,invIdeal,permIdeal,sumIntersectionIdeal,t,posLast,checkIdeal,reductionResult},
Do[
perm=permList[[i]];
...
CheckInvariant[candidates_,rewriteRulesInnerLoops_]:=Module[{dedependencyPolys,newCandidates,wpList,
...
PolyDependencyCheck[{},candidatePolyList_,depPolyList_]:=depPolyList
...
InnerLoopRewwriteRules[{},rewriteRules_]:=rewriteRules


InnerLoopRewwriteRules[{Body[seq__],innerLoops___},rewriteRules_]:=
Module[
{eqs,seqRules,newRules},
eqs=RecEqs[seq,{}];
...
WP[{},poly_]:=Simplify[poly]


WP[{seq___,rule1_},poly_]:=WP[{seq},poly/.rule1]


WPInnerLoops[{},poly_,wpPoly_]:=wpPoly


WPInnerLoops[{innerLoopRules1_,rulesList___},poly_,wpPoly_]:=Module[{wp1},
wp1Poly=WP[innerLoopRules1,poly];
WPInnerLoops[{rulesList},poly,Append[wpPoly,wp1Poly]]
]


WPPolySet[RulesList_,{},wpPolyList_]:=wpPolyList


WPPolySet[RulesList_,{poly1_,polySeq___},wpPolyList_]:=Module[
{wpListPoly1},
